...
```{r}
calculate_distance <- function(state_fuel, postcode_sf, fueltype, date, p_code) {
  ...
  # Assuming pop_density is your raster object of population density
...
pop_density <- rast("D:/UOA WORK/Summer Research/Data_Journalism_ASRS/aus_pd_2020_1km.tif")
...
# Select the fueltype you are interested in
nsw_sf <- select_data(nsw, "U91", "2023-11-12")

# calculate nearest station for each point
nearest_station_code <- sapply(1:n, function(i) {
  st_nearest_feature(random_points_sf[i,], nsw_sf)
})

# calculate distance between each point and its nearest station
distance <- sapply(1:n, function(i) {
  st_distance(random_points_sf[i,], nsw_sf[nearest_station_code[i],])
})
...
print(paste("Average distance to the closest gas station in", p_code, "is", mean(distance$average), "meters"))

print(paste("Average weighted distance to the closest gas station in", p_code, "is", mean(distance$average_weighted), "meters"))
