...
## Extract dataframe of unique address from dataframe of state_fuel

```{r}
extract_address <- function(state_fuel) {
  address <- state_fuel %>%
...
-   Define convenient: within 500m to highway=motorway, highway=trunk, highway=primary
-   Else: inconvenient
...
def mutate_convenient(state_fuel):
    ...
    # Assuming 'extract_address' is a predefined function
...
start = time.time()
print(is_convenient(-12.463, 130.845))
end = time.time()
print(end - start)
```
...
```{r}
mutate_convenient <- function(state_fuel) {
  start_time <- Sys.time()
  # extract list of unique adress
  address <- extract_address(state_fuel)
  
  # check if convenient
  address <- address %>%
    rowwise() %>%
    mutate(convenient = py$is_convenient(lat, long))
  
  # left join to OG df
  state_fuel %>%
    # join shorter df to longer df
    left_join(address, by = c("address", "lat", "long"), relationship = "many-to-many") 
...
print(py$is_convenient(-12.463, 130.845))
...
wa_folder_path <- "fuel-prices/wa"

# Function to extract unique addresses and transform street types
extract_unique_addresses <- function(folder_path) {
  files <- list.files(path = folder_path, pattern = "*.csv", full.names = TRUE)
  
  unique_addresses_df <- data.frame(address = character(), stringsAsFactors = FALSE)
  ...
  unique_addresses_df <- unique(unique_addresses_df)
    return(unique_addresses_df)
}
...
write_csv(unique_addresses_data, "unique_addresses.csv")
