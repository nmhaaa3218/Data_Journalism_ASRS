...
wa_long_lat <- read_csv("output_file.csv", show_col_types = FALSE)

wa <- wa %>%
  left_join(wa_long_lat, by = "address", relationship = "many-to-many")
...
```{r}
nsw <- nsw %>%
  left_join(nsw_address, by = c("address","lat","long"), relationship = "many-to-many")

nt <- nt %>%
  left_join(nt_address, by = c("address","lat","long"), relationship = "many-to-many")

qld <- qld %>%
  left_join(qld_address, by = c("address","lat","long"), relationship = "many-to-many")

sa <- sa %>%
  left_join(sa_address, by = c("address","lat","long"), relationship = "many-to-many")

tas <- tas %>%
  left_join(tas_address, by = c("address","lat","long"), relationship = "many-to-many")

wa <- wa %>%
  left_join(wa_address, by = c("address","lat","long"), relationship = "many-to-many")
```
...
```{r}
# Print unique collection dates
unique_dates <- unique(wa$collection_date)
print(unique_dates)
...
```{r}
plot_points_on_map_second <- function(df, fueltype, collection_date) {
  # Filter the data by fuel type
  df <- df[df$fueltype == fueltype, ]
  print(dim(df))  # Add this line to print dimensions
...
```{r}
# Assuming your data frame is named 'original_data'
copied_data <- copy_to(src = as.data.frame(wa), dest = NULL)
...
```{r}
#wa <- wa %>%
 # select(collection_date, TRADING_NAME, brand, fueltype, price, address, postcode, AREA_DESCRIPTION, REGION_DESCRIPTION, long, lat, convenient)


fueltype <- unique(wa$fueltype)
plot_points_on_map_second(wa,fueltype[1], "2021-31-12") 
```
...
# Combustion fuel price distribution by brand
wa %>%
  filter(fueltype == "E85") %>%  # change fuel type
...
distance2 <- calculate_distance_parallel(wa, postcode_sf, pop_density, "U91", "2023-10-10", p_code, n_trials)
...
```{r}
plot_avg_fuel_price_time(wa, "U91")
```
